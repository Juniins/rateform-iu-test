document.addEventListener("DOMContentLoaded", () => {
  ...
  const stepData = {
    ...
    2: { increment: 34.2, newBalance: 80.29 },
  ...
  function loadVideoScript(type) {
    ...
    if (type === "vsl") {
      scriptSrc =
        "https://scripts.converteai.net/5d9f8480-70ee-4640-ab7d-afc37958aa16/players/68d4935b3776360127868989/v4/player.js";
  ...
  function showVideo(stepNumber) {
    ...
    if (currentDiv) {
      if (stepNumber === 4) {
        // ✅ Ativar modo VSL
        container.classList.add("vsl-active");
        loadVideoScript("vsl");
      } else {
        // 🔒 Desativar modo VSL se voltar
        container.classList.remove("vsl-active");
      }

      currentDiv.style.display = "block";
    }

    // Control evaluation and payment sections visibility
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      e.target.id === "congratulations-continue-btn" ||
      e.target.closest("#congratulations-continue-btn")
    ) {
      goToVSL();
    }
  });
